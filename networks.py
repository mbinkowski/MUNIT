...
class MsImageDis(nn.Module):
    ...
    def __init__(self, input_dim, params):
        ...
        for _, net in enumerate(self.cnns):
            print('NET %d' % _)
            print(net)
    ...
    def calc_dis_loss(self, input_fake, input_real, _mean_fake=torch.mean, _mean_real=torch.mean):
        ...
        for it, (out0, out1) in enumerate(zip(outs0, outs1)):
            if self.gan_type == 'lsgan':
                loss += _mean_fake((out0 - 0)**2) + _mean_real((out1 - 1)**2)
            elif self.gan_type == 'wgan-gp':
                loss += _mean_fake(out1) - _mean_real(out0)
            elif self.gan_type == 'nsgan':
                all0 = Variable(torch.zeros_like(out0.data).cuda(), requires_grad=False)
                all1 = Variable(torch.ones_like(out1.data).cuda(), requires_grad=False)
                loss += _mean_fake(F.binary_cross_entropy(F.sigmoid(out0), all0)) + \
                        _mean_real(F.binary_cross_entropy(F.sigmoid(out1), all1))
            else:
                assert 0, "Unsupported GAN type: {}".format(self.gan_type)
        return loss

    def calc_gen_loss(self, input_fake, input_real=None, _mean_fake=torch.mean, _mean_real=torch.mean):
        if input_real is not None:
            return self.calc_dis_loss(input_real, input_fake, _mean_fake, _mean_real)
        # calculate the loss to train G
        outs0 = self.forward(input_fake)
        loss = 0
        for it, (out0) in enumerate(outs0):
            if self.gan_type == 'lsgan':
                loss += _mean_fake((out0 - 1)**2) # LSGAN
            elif self.gan_type == 'wgan-gp':
                loss += _mean_fake(out0)
            elif self.gan_type == 'nsgan':
                all1 = Variable(torch.ones_like(out0.data).cuda(), requires_grad=False)
                loss += _mean_fake(F.binary_cross_entropy(F.sigmoid(out0), all1))
            else:
                assert 0, "Unsupported GAN type: {}".format(self.gan_type)
        return loss

class MsImageDis2(MsImageDis):
    def __init__(self, input_dim, params):
...
#        cnn_x += [nn.Conv2d(dim, 1, 1, 1, 0)]
    ...
    def forward(self, x):
        outs = [o.view(o.size(0), -1) for o in super(MsImageDis2, self).forward(x)]
        return self.linear(torch.cat(outs, dim=1))
